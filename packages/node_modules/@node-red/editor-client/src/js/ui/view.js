...
RED.view = (function() {
            ...
            add: function(node) {
                if (Array.isArray(node)) {
                    for (var i=0;i<node.length;i++) {
                        api.add(node[i]);
                        console.log("line 127",node)
                    }
                } else {
                    if (!setIds.has(node.id)) {
                        set.push({n:node});
                        setIds.add(node.id);
                        console.log("line 133",node.id)
                    }
                }
            },
    ...
    function init() {
        ...
        chart.droppable({
            ...
            drop: function( event, ui ) {
                ...
                console.log("slected tool in view.js",selected_tool)
    ...
    function addNode(type,x,y) {
        ...
        // if(!def){
        //    def={ category: "own-custom-node",
        //     color: "#8787",
        //     // defaults: {nameone: {…}, ip: {…}, port: {…}, host: {…}, testIP: {…}},
        //     icon: "gitlab.png",
        //     inputs: 1,
        //     label: ƒ (),
        //     outputs: 1,
        //     set: {id: "node-red-contrib-example-gitlab-case/gitlab", name: "gitlab", types: Array(1), enabled: true, local: true},
        //     type: "gitlab"
        // }}
        console.log("nn in view.js 880",nn)

           
        nn.type = type;
        console.log("nn.type in view.js 884",nn.type)
        nn._def = RED.nodes.getType(nn.type);
        console.log("nn._def in view.js 884",nn._def)
        if (!m) {
